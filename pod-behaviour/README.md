...
## Pod termination
In this and the following sections, we will see how containers interact with the Kubernetes machinery. This will require Kubernetes so ensure that the kind cluster is running. Run `kind load docker-image entrypoint-exec:0.0.1 --name k8splayground` to load the entrypoint-exec:0.0.1 image to the kind "hosts" - this is the image all of our Pod manifests will use unless indicated otherwise.
