...
To search the logs emitted by Fluent Bit and indexed by Elasticsearch, one must first define an Index Pattern (from Stack Management -> Index Patterns). An Index Pattern is a regex that describes which Elasticsearch indices should be searched. Since we've configured our Fluent Bit-generated indices to have "logs" as a prefix, the regex we'll use for our Index Pattern is "logs-*". Then, in Discover, with the Index Pattern we just created, we can use a query like `kubernetes.labels.app:kibana` to search for logs from Pods with the label "app=kibana".

If prometheus-operator is running, Fluent Bit metrics will be viewable in Prometheus.
